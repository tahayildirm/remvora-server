...
## Terminal privilege policy extension (agent 0.3.7+)

Server-originated `session.request` includes `allowTerminalPrivilegeEscalation` (boolean, default false). It is derived from the persisted device setting AND the authenticated operator's `devices.terminalElevation` permission; browser input never sets it. A terminal `session.accept` must acknowledge `terminalPolicyVersion: 1` and `terminalPrivilegeEscalation` matching the requested value. Missing/mismatched acknowledgement yields `TERMINAL_POLICY_AGENT_UPDATE_REQUIRED`; the session is closed. Unsupported local elevation yields `TERMINAL_ELEVATION_UNAVAILABLE`. Update agents before deploying this server change. Changing the policy closes terminal sessions while preserving the agent control connection and desktop sessions, and live authorization is rechecked. This setting controls Linux no-new-privileges inheritance, not OS account authorization or a general sandbox.
